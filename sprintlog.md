...
| **52b** | 🟠 P8 | FEAT | **Leerling** — **Klas-startcode** (`classes.start_code` + `start_code_active`): leerkracht genereert een board-leesbare code (6 tekens, geen 0/1/O/I) en kan het venster sluiten/heropenen. Backend + endpoints ✅; display + open/dicht + ↻ in het klasbeheer ✅ (deel 2). | ~0.5 dag |
...
| **52e** | 🔴 P10 | SEC | **Leerling** — **Toegangsregel** (de kern van je ontwerp): `pending` mag **wél** een klassessie en vrij oefenen (les nooit onderbreken), maar **géén toets/taak**. `blocked` mag niets. `quiz_start` controleert nu de status. *Pure regel `magLeerlingActiviteit` + tests ✅ (deel 1); afdwinging in `quiz_start` ✅ (deel 3).* | ~0.5 dag |
