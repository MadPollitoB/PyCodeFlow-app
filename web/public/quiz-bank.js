  ...
  el.innerHTML = qs.map((q, idx) => {
    ...
    <div class="q-card ${q.archived ? 'archived-card' : ''}" data-qid="${q.id}">
      ...
      <div style="margin:6px 0 2px;display:flex;align-items:center;gap:6px;">
        ...
        <select class="q-scope-select" data-prev="${q.share_scope || 'private'}"
                onchange="setQuestionScope('${q.id}', this)"
                style="padding:4px 8px;border:1.5px solid var(--border);border-radius:8px;font-size:0.8rem;background:var(--surface);color:var(--text);">
          <option value="private"${(q.share_scope||'private')==='private'?' selected':''}>🔒 Privé</option>
          <option value="school"${q.share_scope==='school'?' selected':''}>🏫 School</option>
          <option value="public"${q.share_scope==='public'?' selected':''}>🌍 Publiek</option>
        </select>
      </div>` : ''}
...
// Sprint 51c: zichtbaarheid van een eigen vraag wijzigen vanuit de Vragenbank.
// Dit is het startpunt om een privé-vraag te delen (school/publiek). De server
// weigert (409) als de vraag al aan een sjabloon hangt — dan zetten we terug.
async function setQuestionScope(id, sel) {
  const scope = sel.value, prev = sel.getAttribute('data-prev');
  try {
    const r = await apiFetch(`/api/quiz/bank/${id}/scope`, {
      method: 'PUT', body: JSON.stringify({ scope }),
    });
    const d = await r.json().catch(() => ({}));
    if (r.ok && d.ok) {
      sel.setAttribute('data-prev', scope);
      if (window.pyToast) pyToast('Zichtbaarheid aangepast.', 'success');
    } else {
      sel.value = prev;
      if (window.pyAlert) pyAlert(d.error || 'Wijzigen mislukt.', 'error');
    }
  } catch (e) {
    sel.value = prev;
    if (window.pyAlert) pyAlert('Wijzigen mislukt.', 'error');
  }
}
