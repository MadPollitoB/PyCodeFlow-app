...
# ── Sprint 54: testdatabase-seeder ───────────────────────────────────────────
actie_testdb() {
  local WEB_CONT="pycodeflow-web-1"
  while true; do
    header
    echo -e "${BOLD}  ╔══════════════════════════════════════════╗${RESET}"
    echo -e "${BOLD}  ║  🧬  Testdatabase (seed / wis)           ║${RESET}"
    echo -e "${BOLD}  ╚══════════════════════════════════════════╝${RESET}"
    echo ""
    echo -e "  ${GEEL}⚠  ENKEL VOOR TEST/STAGING — NOOIT OP PRODUCTIE.${RESET}"
    echo -e "  ${DIM}Alle rijen zijn gemarkeerd (testdata-/TD…/TESTDATA) en wachtwoorden${RESET}"
    echo -e "  ${DIM}zijn gelijk aan de gebruikersnaam. De wis-optie verwijdert enkel die rijen.${RESET}"
    echo ""
    echo -e "  Inloggegevens na seed (wachtwoord = gebruikersnaam):"
    echo -e "    Leerkrachten: ${BOLD}superadmin${RESET} · ${BOLD}leerkrachtA${RESET} (admin, school A) · ${BOLD}leerkrachtA2${RESET} · ${BOLD}leerkrachtB${RESET} (admin, school B)"
    echo -e "    Leerlingen:   ${BOLD}studentA@testschool.local${RESET} (actief) · studentA2 (pending) · studentA3 (geblokkeerd)"
    echo -e "    Klascodes:    TDKLAS5A · TDKLAS6A · TDKLAS5B — sessies: TDLESA5 / TDTOETSA / TDTAAKA"
    echo ""
    echo -e "  ${BOLD}1)${RESET} Testdata aanmaken ${DIM}(idempotent — 2× draaien kan geen kwaad)${RESET}"
    echo -e "  ${BOLD}2)${RESET} Status — hoeveel testdata staat er?"
    echo -e "  ${BOLD}3)${RESET} Testdata wissen"
    echo -e "  ${BOLD}0)${RESET} Terug"
    echo ""
    read -rp "  Keuze: " tk
    echo ""
    case "$tk" in
      1)
        read -rp "  Typ SEED om te bevestigen dat dit GEEN productie is: " bevestig
        if [[ "$bevestig" == "SEED" ]]; then
          docker exec "$WEB_CONT" node /app/scripts/seed-testdb.js seed \
            || err "Seeden mislukt — draait de web-container? (optie 2: Start)"
        else
          warn "Niet bevestigd — geannuleerd."
        fi
        pauze ;;
      2) docker exec "$WEB_CONT" node /app/scripts/seed-testdb.js status || err "Container niet bereikbaar."; pauze ;;
      3)
        read -rp "  Typ WIS om de testdata te verwijderen: " bevestig
        if [[ "$bevestig" == "WIS" ]]; then
          docker exec "$WEB_CONT" node /app/scripts/seed-testdb.js wipe \
            || err "Wissen mislukt — draait de web-container?"
        else
          warn "Niet bevestigd — geannuleerd."
        fi
        pauze ;;
